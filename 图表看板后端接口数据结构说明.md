...
| failRates | Number[] | 对应年份的不及格率（**百分比形式**） | [11.5, 10.2, 9.5, 8.8, 8.0] |
...
- `failRates[i]`：该年度的不及格率（百分比，`100 - passRates[i]`）
...
- 不及格率：`((1 - currentMetrics.passRate) * 100).toFixed(1) + '%'`
