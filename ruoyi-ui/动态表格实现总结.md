...
成功将教师成绩和个人成绩中的固定表格改为动态多层级表头表格，支持通过JSON配置文件自定义表头结构，并提供默认配置作为后备方案。
...
├── config/
│   └── tableHeaders/
│       ├── default-teacher.json       # 教师成绩默认配置
│       ├── default-leader.json        # 个人成绩默认配置
│       └── teacher-2024-0001.json     # 自定义配置示例
├── utils/
│   └── tableHeaderConfig.js           # 配置管理工具类
├── components/Dashboard/
│   ├── TeacherDashboard.vue           # 教师成绩（已修改）
│   └── LeaderDashboard.vue            # 个人成绩（已修改）
└── views/test/
